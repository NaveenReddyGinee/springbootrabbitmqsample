...
import com.accenture.service.MessageSender;
...
public class UserService {
	...
	@GetMapping(path="/sendmessage/{mssage}",produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<String> sendMessage(@PathVariable("mssage") String mssage){
		
		String exchange = getApplicationConfig().getApp1Exchange();
		String routingKey = getApplicationConfig().getApp1RoutingKey();
		
		try {
			messageSender.sendMessage(rabbitTemplate, exchange, routingKey, mssage);
			return new ResponseEntity<String>(ApplicationConstant.IN_QUEUE, HttpStatus.OK);
	...
	@GetMapping(path="/consume/{msg}",produces=MediaType.APPLICATION_JSON_VALUE)
